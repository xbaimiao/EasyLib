...
private class ResettableLazyImpl<T>(vararg groups: String, initializer: () -> T) : ResettableLazy<T>(*groups) {
    ...
    override fun toString() =
        if (isInitialized()) value.toString() else "Lazy(${groups.joinToString()}) value not initialized yet."
}

private class ResettableSynchronizedLazyImpl<T>(vararg groups: String, initializer: () -> T) :
    ResettableLazy<T>(*groups) {
